...
const BILLER_KEYWORDS: Record<string, string> = {
  "ডেসকো": "ডেসকো",
  "ডেস্কো": "ডেসকো",
  "পল্লী": "পল্লী বিদ্যুৎ",
  "পল্লি": "পল্লী বিদ্যুৎ",
...
export async function classify(
  ...
  switch (expectedType) {
    ...
    case "biller_name_or_tap":
      result = classifyBiller(text, base);
      break;
    ...
    case "account_number":
      result = classifyAccountNumber(text, base);
      break;
...
function classifyBiller(
  text: string,
  base: Omit<InputClassification, "type" | "confidence">
): InputClassification {
  if (BILLER_MAP[text.toLowerCase().trim()]) {
    return {
      ...base,
      type: "valid_slot",
      extracted_slot: BILLER_MAP[text.toLowerCase().trim()],
      slot_type: "biller",
      confidence: 0.95,
    };
  }
  for (const [keyword, banglaName] of Object.entries(BILLER_KEYWORDS)) {
    if (text.includes(keyword)) {
      return {
        ...base,
        type: "valid_slot",
...
const BANGLA_TO_ASCII_DIGITS: Record<string, string> = {
  "০": "0", "১": "1", "২": "2", "৩": "3", "৪": "4",
  "৫": "5", "৬": "6", "৭": "7", "৮": "8", "৯": "9",
};

function classifyAccountNumber(
  text: string,
  base: Omit<InputClassification, "type" | "confidence">
): InputClassification {
  const digits = text
    .replace(/[০-৯]/g, (d) => BANGLA_TO_ASCII_DIGITS[d] ?? d)
    .replace(/\D/g, "");
  // Biller/account numbers vary (e.g. DESCO 8-10, WASA meter, etc.). Accept 5–17 digits.
  if (digits.length >= 5 && digits.length <= 17) {
    return {
      ...base,
      type: "valid_slot",
      extracted_slot: digits,
      slot_type: "account_number",
      confidence: 0.9,
    };
  }
  return { ...base, type: "unrecognized", confidence: 0.4 };

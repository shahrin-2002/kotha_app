...
export async function llmClassify(
  ...
  if (expectedType === "intent") {
    systemPrompt = `You classify Bangla MFS voice commands. The user wants to do one of: send_money, cash_out, recharge, check_balance, pay_bill.
They may say things like "টাকা পাঠাবো", "ক্যাশ আউট করবো", "রিচার্জ করবো", "ব্যালেন্স দেখবো", "বিল দিব"/"বিদ্যুৎ বিল দিব" or similar.
They might also say: cancel (বাতিল/থামো), help (সাহায্য/কী করতে হবে), or repeat (আবার বলো).
Respond with ONLY one of: send_money, cash_out, recharge, check_balance, pay_bill, cancel, help, repeat, unknown`;
...
Respond with ONLY one of: yes, no, cancel, change, unknown`;
  ...
  try {
    ...
    if (expectedType === "intent") {
      ...
      if (["send_money", "cash_out", "recharge", "check_balance", "pay_bill"].includes(result)) {

...
import { SelectBillerPage } from "./pages/SelectBillerPage";
...
function App() {
  ...
  const handleBillerSelect = useCallback(
    (biller: string) => {
      session.sendTap("biller_select", biller);
    },
    [session.sendTap]
  );
  ...
  const handleAccountSubmit = useCallback(
    (accountNumber: string) => {
      session.sendTranscript(accountNumber);
    },
    [session.sendTranscript]
  );
  ...
  return (
    <>
      ...
      {screen !== "pin_pad" && screen !== "enter_number" && screen !== "enter_account" && (
        <MicButton
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", justifyContent: "center", marginBottom: "0.4rem" }}>
          ...
          {screen === "select_biller" && <>
            <button className="quick-btn" onClick={() => handleQuickPhrase("ডেসকো")}>ডেসকো</button>
            <button className="quick-btn" onClick={() => handleQuickPhrase("পল্লী বিদ্যুৎ")}>পল্লী বিদ্যুৎ</button>
            <button className="quick-btn" onClick={() => handleQuickPhrase("তিতাস গ্যাস")}>তিতাস গ্যাস</button>
            <button className="quick-btn" onClick={() => handleQuickPhrase("ঢাকা ওয়াসা")}>ঢাকা ওয়াসা</button>
          </>}
          {screen === "select_recipient" && <>
